var JSMpeg = {
    ...
    Fill: function(array, value) {
        ...
        else for(var i = 0; i < array.length; i++)array[i] = value;
    },
...
JSMpeg.Player = function() {
    ...
    Player.prototype.updateForStaticFile = function() {
        ...
        } else if (notEnoughData && this.options.onStalled) this.options.onStalled(this);
    };
    return Player;
}();
JSMpeg.BitBuffer = function() {
    ...
    var BitBuffer = function(bufferOrLength, mode) {
        if (typeof bufferOrLength === "object") {
            ...
            this.bytes = new Uint8Array(bufferOrLength || 1048576);
            this.byteLength = 0;
        }
    ...
    BitBuffer.prototype.write = function(buffers) {
        ...
        if (isArrayOfBuffers) for(var i = 0; i < buffers.length; i++)this.appendSingleBuffer(buffers[i]);
    ...
    BitBuffer.prototype.findNextStartCode = function() {
        for(var i = this.index + 7 >> 3; i < this.byteLength; i++)if (this.bytes[i] == 0 && this.bytes[i + 1] == 0 && this.bytes[i + 2] == 1) {
            this.index = i + 4 << 3;
...
JSMpeg.Demuxer.TS = function() {
    ...
    TS.prototype.write = function(buffer) {
        ...
        while(this.bits.has(1504) && this.parsePacket());
    ...
    TS.prototype.resync = function() {
        if (!this.bits.has(9024)) return false;
        var byteIndex = this.bits.index >> 3;
        for(var i = 0; i < 187; i++)if (this.bits.bytes[byteIndex + i] === 71) {
            var foundSync = true;
            for(var j = 1; j < 5; j++)if (this.bits.bytes[byteIndex + i + 188 * j] !== 71) {
                foundSync = false;
                break;
            }
            if (foundSync) {
...
JSMpeg.Decoder.MPEG1Video = function() {
    ...
    var MPEG1 = function(options) {
        ...
        var bufferSize = options.videoBufferSize || 524288;
    ...
    MPEG1.prototype.readHuffman = function(codeTable) {
        var state = 0;
        do state = codeTable[state + this.bits.read(1)];
    ...
    MPEG1.prototype.decodePicture = function(skipOutput) {
        ...
        var code = 0;
        do code = this.bits.findNextStartCode();
    ...
    MPEG1.prototype.decodeMotionVectors = function() {
        ...
        if (this.macroblockMotFw) {
            ...
            else if (this.motionFwHPrev < -this.forwardF << 4) this.motionFwHPrev += this.forwardF << 5;
            ...
            else if (this.motionFwVPrev < -this.forwardF << 4) this.motionFwVPrev += this.forwardF << 5;
    ...
    MPEG1.prototype.decodeBlock = function(block) {
        ...
        while(true){
            ...
            if (coeff === 65535) {
                ...
                else if (level > 128) level = level - 256;
            } else {
                run = coeff >> 8;
                level = coeff & 255;
    ...
    MPEG1.IDCT = function(block) {
        ...
        for(var i = 0; i < 8; ++i){
            b1 = block[32 + i];
            b3 = block[16 + i] + block[48 + i];
            b4 = block[40 + i] - block[24 + i];
            tmp1 = block[8 + i] + block[56 + i];
            tmp2 = block[24 + i] + block[40 + i];
            b6 = block[8 + i] - block[56 + i];
            b7 = tmp1 + tmp2;
            m0 = block[0 + i];
            x4 = (b6 * 473 - b4 * 196 + 128 >> 8) - b7;
            x0 = x4 - ((tmp1 - tmp2) * 362 + 128 >> 8);
            x1 = m0 - b1;
            x2 = ((block[16 + i] - block[48 + i]) * 362 + 128 >> 8) - b3;
            x3 = m0 + b1;
            y3 = x1 + x2;
            y4 = x3 + b3;
            y5 = x1 - x2;
...
JSMpeg.Decoder.MPEG1VideoWASM = function() {
    ...
    var MPEG1WASM = function(options) {
        ...
        this.bufferSize = options.videoBufferSize || 524288;
...
JSMpeg.Decoder.MP2Audio = function() {
    ...
    var MP2 = function(options) {
        ...
        var bufferSize = options.audioBufferSize || 131072;
        var bufferMode = options.streaming ? JSMpeg.BitBuffer.MODE.EVICT : JSMpeg.BitBuffer.MODE.EXPAND;
        this.bits = new JSMpeg.BitBuffer(bufferSize, bufferMode);
        ...
        for(var j = 0; j < 2; j++)for(var i = 0; i < 32; i++){
    ...
    MP2.prototype.decodeFrame = function(left, right) {
        ...
        for(var sb = 0; sb < sblimit; sb++){
            for(ch = 0; ch < channels; ch++)if (this.allocation[ch][sb]) this.scaleFactorInfo[ch][sb] = this.bits.read(2);
            if (mode === MP2.MODE.MONO) this.scaleFactorInfo[1][sb] = this.scaleFactorInfo[0][sb];
        }
        for(var sb = 0; sb < sblimit; sb++){
            for(var ch = 0; ch < channels; ch++)if (this.allocation[ch][sb]) {
        ...
        var outPos = 0;
        for(var part = 0; part < 3; part++)for(var granule = 0; granule < 4; granule++){
            for(var sb = 0; sb < bound; sb++){
...
JSMpeg.Decoder.MP2AudioWASM = function() {
    ...
    var MP2WASM = function(options) {
        ...
        this.onDecodeCallback = options.onAudioDecode;
        this.module = options.wasmModule;
        this.bufferSize = options.audioBufferSize || 131072;
        this.bufferMode = options.streaming ? JSMpeg.BitBuffer.MODE.EVICT : JSMpeg.BitBuffer.MODE.EXPAND;
        this.sampleRate = 0;
    };
...
//# sourceMappingURL=index.2284d008.js.map
